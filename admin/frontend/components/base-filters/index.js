  ...
  return (<div className='panel-block' key={key}>
    <div className='field c-flex-1'>
      <p className='control has-icons-left is-expanded'>
        <input className='input is-small' name={name} type='text' value={value} onChange={handleInputChange} placeholder={title} />
...
class BaseFilterPanel extends Component {
  ...
  getFilters (type, title, key, value, name, handleInputChange) {
    let filters = ({
      SearchFilter
    })[ type ] || SearchFilter

    return filters(title, key, value, name, handleInputChange)
  }
    ...
    return <div className='card full-height is-shadowless'>
      ...
      <form >
        {Object.keys(uiSchema).map((e, i) => {
          return this.getFilters(uiSchema[e]['ui:widget'], schema.properties[e].title, i, this.state.formData[e], e, this.handleInputChange)
        })}
